...
export async function extractDocument(
  ...
  const document = services.documents.LangiumDocuments.getOrCreateDocument(
    URI.file(path.resolve(fileName))
  );
  const buildResult = await services.documents.DocumentBuilder.build(document);

  const validationErrors = buildResult.diagnostics.filter(
    (e) => e.severity === 1
  );
...
export async function extractAstNode<T extends AstNode>(
  ...
  return (await extractDocument(fileName, extensions, services)).parseResult
    ?.value as T;
}
